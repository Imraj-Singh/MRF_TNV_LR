...
%% Perform TNV reconstruction

for eta = sqrt([10 5 2.5 1])
    maxit  = 20000;
    theta = 1;
    sig = 0.35;
    tau = 0.35;
    ...
    for i = 1:maxit
        ...
        if abs(data_fit_func - eta^2) < 0.01
            break
        end
    ...
    save(['TNV_test_', num2eng(eta^2),'.mat'],'ubar', 'data_fit_func','nuc_norm_func')
end
%%

function sum_s = nuc_norm(u, z)
Z = Op_grad(u,z);
[S, ~] = SV_decomp(Z);
sum_s = sum(abs(S(:)));
end
