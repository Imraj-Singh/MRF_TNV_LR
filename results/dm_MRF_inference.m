...
%%
disp('performing dictionary matching quantitative inference...');
...
out.qmap = double(reshape(out.qmap, [recon_dim(1:end-1), size(dict.lut,2)]));
    
Dx = sum(conj(D) .* x ,2);
Dx  = reshape(Dx,   recon_dim(1:end-1));
out.pd = Dx(:) ./ dict.normD(idx);
out.pd  = double(reshape(out.pd,   recon_dim(1:end-1)));
